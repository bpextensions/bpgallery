...
class ImageRouterRulesLegacy implements JComponentRouterRulesInterface
{
	...
	/**
	 * Preprocess the route for the com_bpgallery component
	 *
	...
	/**
	 * Build the route for the com_bpgallery component
	 *
	 * @param   array  &$query     An array of URL arguments
	...
	public function build(&$query, &$segments)
	{
		...
		if (isset($query['view']))
		{
			...
			if (empty($query['Itemid']) || empty($menuItem) || $menuItem->component != 'com_bpgallery')
			{
				$segments[] = $query['view'];
			}
		...
		if (isset($view) and ($view == 'category' or $view == 'image'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				...
				$menuCatid = $mId;
				$categories = JCategories::getInstance('BPGallery');
	...
	public function parse(&$segments, &$vars)
	{
		...
		$imageCategory = JCategories::getInstance('BPGallery')->get($id);

		$categories = $imageCategory ? $imageCategory->getChildren() : array();
		$vars['catid'] = $id;
		$vars['id'] = $id;
		$found = 0;
